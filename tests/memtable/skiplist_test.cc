...
#include "../test_util/test_harness.hh"
...
class ConcurrentTest {
 private:
  ...
  static uint64_t HashNumbers(uint64_t k, uint64_t g) {
    ...
    return badger::hash(reinterpret_cast<char*>(data), sizeof(data), 0);
  }
...
class TestState {
 public:
  ...
  explicit TestState(int s) : seed_(s), quit_flag_(false), state_(STARTING) {}

  void Wait(ReaderState s) {
    std::unique_lock lock_guard{mu_};

    while (state_ != s) state_cv_.wait(lock_guard);
  }
...
static void RunConcurrent(int run) {
  const int seed = test::random_seed() + (run * 100);
  Random rnd(seed);
  const int N = 1000;
  ...
  for (int i = 0; i < N; i++) {
    if ((i % 100) == 0) fprintf(stderr, "Run %d of %d\n", i, N);

    auto state = std::make_shared<TestState>(seed + 1);
    pool.Schedule([state]() { ConcurrentReader(state.get()); });
