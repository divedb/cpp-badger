...
std::string Random::HumanReadableString(int len) {
  ...
  for (int i = 0; i < len; ++i) ret[i] = static_cast<char>('a' + Uniform(26));
...
std::string Random::RandomString(int len) {
  ...
  for (int i = 0; i < len; i++) ret[i] = static_cast<char>(' ' + Uniform(95));
...
std::string Random::RandomBinaryString(int len) {
  ...
  for (int i = 0; i < len; i++) ret[i] = static_cast<char>(Uniform(CHAR_MAX));
