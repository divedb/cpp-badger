...
template <typename Hash, typename Range>
inline Range fast_range_generic(Hash hash, Range range) {
...
inline size_t fast_range64(uint64_t hash, size_t range) {
  return fast_range_generic(hash, range);
}

// Map a quality 32-bit hash value down to an arbitrary uint32_t range.
inline uint32_t fast_range32(uint32_t hash, uint32_t range) {
  return fast_range_generic(hash, range);
}
