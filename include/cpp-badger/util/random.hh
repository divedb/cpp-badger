...
/// package.
class Random {
  enum : uint32_t { M = 2147483647L };

  // bits 14, 8, 7, 5, 2, 1, 0
  // Multiplier used in the linear congruential generator.
  enum : uint64_t { A = 16807 };

  /// Ensures the seed is valid (non-zero and within range).
  ///
  /// \param s The input seed value.
  /// \return A corrected seed guaranteed to be in the valid range [1, M).
 ...
 public:
  ...
  /// \return Returns a Random instance for use by the current thread without
  ///         additional locking.
  static Random* GetTLSInstance();

  /// Constructs a new Random instance with the given seed.
  ///
  /// \param s Seed value.
  ...
  ///
  /// \return A random 31-bit unsigned integer in [1, M).
  uint32_t Next() {
    // We are computing
  ...
  /// Generates a 64-bit random value.
  ///
  ...
  ///
  /// \param n Denominator for probability (must be > 0).
  /// \return True approximately 1 out of n times, otherwise false.
  ...
  ///
  /// \param n Denominator for probability.
  /// \return True approximately 1 out of n times, otherwise false.
  ...
  ///
  /// \param percentage Chance of returning true (0 = always false, 100 = always
  ///                   true).
  /// \return Random boolean with expected true probability = percentage%.
  ...
  ///
  /// \param max_log Maximum exponent for the distribution.
  /// \return Random integer in range [0, 2^max_log - 1], biased exponentially.
  uint32_t Skewed(int max_log) { return Uniform(1 << Uniform(max_log + 1)); }

  /// Generates a random string of given length.
  ///
  /// \param len Desired string length.
  /// \return Random string containing arbitrary characters.
  std::string RandomString(int len);

  /// Generates a random string of given length using human-readable characters.
  ///
  /// \param len Desired string length.
  /// \return Random string consisting of readable alphanumeric characters.
  std::string HumanReadableString(int len);

  /// Generates a random binary string.
  ///
  /// \param len Desired string length.
  /// \return Random string containing binary data (may include null bytes).
...
void random_shuffle(RandomIt first, RandomIt last, uint32_t seed) {
...
void random_shuffle(RandomIt first, RandomIt last) {
  random_shuffle(first, last, std::random_device{}());
}
