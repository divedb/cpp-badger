...
class SkipList {
 ...
 public:
  ...
  static constexpr int kDefaultBranchingFactor = 4;
  ...
  ///
  /// \param cmp Comparator used to order the elements in the SkipList.
  /// \param allocator Memory allocator for node allocation.
  /// \param max_height Maximum number of levels the SkipList can have.
  /// \param branching_factor Controls the probability of a node being promoted
  ///                         to a higher level. Each node has a
  ///                         1/branching_factor chance to be promoted. A
  ///                         smaller branching factor creates taller skip lists
  ///                         (faster lookups, more memory), while a larger
  ///                         branching factor creates flatter skip lists
  ///                         (slower lookups, less memory).
  explicit SkipList(Comparator cmp, Allocator* allocator,
                    int32_t max_height = kDefaultMaxHeight,
                    int32_t branching_factor = kDefaultBranchingFactor);
 ...
 private:
  ...
  /// Modified only by Insert().  Read racily by readers, but stale values are
  /// ok.
...
SkipList<Key, Comparator>::SkipList(const Comparator cmp, Allocator* allocator,
  ...
  prev_ = reinterpret_cast<Node**>(
      allocator_->Allocate(sizeof(Node*) * kMaxHeight_));
